...
def get_rollout(train_state, config):
    ...
    while not done:
        ...
        obs, state, rewards, dones, infos = env.batch_step(key_s, state, actions_dict)
        

        done = dones['__all__']
        state_seq.append(state)
    print(f'State sequence length: {len(state_seq)}')
    return state_seq
'''

def get_reset(config):
    # Initialize the environment
    env = make(config["env"]["ENV_NAME"], **config["env"]["ENV_KWARGS"])
    env = CTRolloutManager(env, 1)
...
def main(config):
    ...
    state = get_reset(config)
    viz = OvercookedVisualizer()
    viz.animate(state, agent_view_size=5, filename=f'{config["env"]["ENV_NAME"]}_vdn.gif')
